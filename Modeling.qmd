...
logistic_model_all_main
summary(logistic_model_all_main)
...
# Print training model fit
logistic_model_sig_main
summary(logistic_model_sig_main)
...
# Print training model fit
logistic_model_sig_main_interaction
summary(logistic_model_sig_main_interaction)

```

> ### Logistic regression results
>
> Overall the model with all of the main effects has the best performance in terms of minimizing logloss
> The model with interactions and main effects has a slightly higher logloss and significantly longer training time, no benefit
...
#tree_model <- readRDS("tree_7262024.rds")
...
loaded_model <- readRDS("random_forest_model_7262024.rds")
...
review_data$Diabetes_binary_numeric <- ifelse(review_data$Diabetes_binary == "Yes", 1, 0)

# Generate probabilities so I can calculate log-loss
review_data$Diabetes_logistic_prob <- predict(logistic_model_all_main, newdata = review_data, type = "prob")[,2]
...
# Generate prediction
review_data$Diabetes_logistic <- predict(logistic_model_all_main, newdata = review_data)
...
# Custom function to calculate log-loss
log_loss <- function(actual, predicted_prob) {
  eps <- 1e-15
  predicted_prob <- pmin(pmax(predicted_prob, eps), 1 - eps)
  -mean(actual * log(predicted_prob) + (1 - actual) * log(1 - predicted_prob))
}

# Print logloss and confusion matrix for each
cat("Log-Loss for Logistic Regression:", log_loss(review_data$Diabetes_binary_numeric, review_data$Diabetes_logistic_prob), "\n")
confusionMatrix(review_data$Diabetes_binary, review_data$Diabetes_logistic)

cat("Log-Loss for Classification Tree:", log_loss(review_data$Diabetes_binary_numeric, review_data$Diabetes_tree_prob), "\n")
confusionMatrix(review_data$Diabetes_binary, review_data$Diabetes_tree)

cat("Log-Loss for Random Forest:", log_loss(review_data$Diabetes_binary_numeric, review_data$Diabetes_rf_prob), "\n")
confusionMatrix(review_data$Diabetes_binary, review_data$Diabetes_rf)
